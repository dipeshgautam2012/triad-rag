...
Three services (retrieval, orchestrator, generation) and three UIs. They talk over HTTP only — no shared Python code between services.
...
| **Eval UI** (`eval/eval_ui.py`) | Orchestrator + retrieval + generation — golden-set metrics |
...
Batch-check your RAG pipeline against a **golden set**: fixed questions with reference answers and expected source locations. Eval calls the same HTTP APIs as chat (`/retrieve`, `/query`, and `/generate` for the faithfulness judge) and writes a versioned CSV report.

### What it measures

For each golden question, eval:

1. **Search** the collection and take the first N passages (N = **top_k**, default 5).
2. **Check retrieval** — did the expected file (and PDF page, if you set one) appear in those results?
3. **Ask the chat pipeline** for an answer (same as Chat UI).
4. **Optionally check faithfulness** — a second LLM call asks whether that answer is supported by what was retrieved.

Summary numbers are printed at the end and saved in `summary.json`.

### Metrics

| Metric | What it means |
|--------|----------------|
| **Retrieval match** | `1` if the expected file (and page, when set) appears in the search results; otherwise `0`. Report column: `hit_at_k`. |
| **Keyword overlap** | Fraction of words from your `ground_truth` answer that also appear in the retrieved text (0 to 1). |
| **Faithfulness** | `1` if the judge says the chat answer is supported by retrieved text; `0` if not. Blank if faithfulness was skipped or could not run. |
| **faithfulness_reason** | Short note from the judge. |
| **answer** | Chat answer (truncated in the CSV). |
| **notes** | e.g. faithfulness skipped. |
| **error** | Something failed for that question (HTTP error, etc.). |

Faithfulness needs a **real LLM** (e.g. Google in `env.toml`). With **stub**, search and chat answers still run; turn on **Skip faithfulness** in the UI or use `--skip-faithfulness` on the CLI.

### Dataset layout

```
eval/datasets/<dataset_id>/
  golden.jsonl          # track in git
  reports/              # generated by eval — gitignored
    <YYYYMMDDTHHMMSS>/
      report.csv
      summary.json
```

Example: `eval/datasets/my_dataset/golden.jsonl`.

Each line in `golden.jsonl` is one test case:
...
|-------|-----------|------------|
| `question` | **Yes** | The question sent to search and chat. |
| `ground_truth` | **Yes** | The correct answer you wrote by hand. |
| `expected_source` | No | Filename from upload (e.g. `handbook.pdf`, `notes.txt`). Must match exactly. If you omit it, retrieval match is always `0` for that row. |
| `expected_page` | No | **PDF only.** Page number: first page = `1`, second = `2`, and so on. If you omit it, any page from that PDF counts as a match. |

**PDF vs `.txt`**

- **PDF:** each chunk knows its filename and page number. You can set `expected_source` and optionally `expected_page`.
- **`.txt`:** chunks only know the filename — **there is no page**. Set `expected_source` only; do not set `expected_page`.

Before running eval, ingest every file listed in `expected_source` into the collection you query (default collection name = dataset folder name).

### Eval UI

Services must be running (retrieval, orchestrator, generation).
...
- **Dataset** — folder under `eval/datasets/` with `golden.jsonl`, or upload a one-off file.
- **Index** — collection to query (defaults to dataset id).
- **Provider / model** — applied via `POST /models/select` before each run (same as Chat UI).
- **Past report versions** — load older runs from `reports/<timestamp>/`.
- **Skip faithfulness** — retrieval + answer metrics only.

The UI validates that `expected_source` files exist in the collection before running.

### Eval CLI

Uses the **currently active** provider on the generation service (from `env.toml` on restart, or last `POST /models/select`).
...
| Flag | Default | Notes |
|------|---------|-------|
| `--dataset` | `top_llm_questions` | Folder name under `eval/datasets/` (repo includes `top_llm_questions` as an example) |
| `--golden` | `<dataset>/golden.jsonl` | Override golden file path |
| `--output` | new timestamped run dir | Override report CSV path |
| `--index-id` | same as `--dataset` | Retrieval collection |
| `--top-k` | `5` | Passages per question |
| `--skip-faithfulness` | off | Skip LLM judge |

Optional env vars (UI and CLI): `RETRIEVAL_API_URL`, `ORCHESTRATOR_API_URL`, `GENERATION_API_URL`.
...
| Run eval (UI) | `streamlit run eval/eval_ui.py` (from repo root; services up) |
